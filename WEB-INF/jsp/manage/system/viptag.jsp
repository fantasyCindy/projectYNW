  ...
  <head>
    ...
    function saveOrUpdate(){
	  ...
	  var _params = $('#formId').serialize();
   	  ...
   	  var _postUrl = '/viptag/addoredit.do';
	   ...
	   function deletes(){
		   ...
		   showConfirm('确认要删除这'+rowDatas.length+'个项目吗？',
			function(){
			   ...
			   $.post(path + '/viptag/del.do',{ids:_ids.toString()},function(_backData){
				   if("success" == _backData){
		   	  			showOnlyMsg('删除成功！');
		   	  			reflushDataTable();
		   	  	   }else
			        ...
			        <form class="form-inline" id="formId">
					    ...
					    <fieldset>
					     ...
					     <div class="form-group">
						    <label>标签名称：</label>
						    <input type="text" style="width: 150px; height: 25px;" id="name" name="tag_name">
						  </div><br/>
						  <div class="form-group">
						    <label>时间简介：</label>
						    <textarea rows="3" cols="50" name="time_profile"></textarea>
						  </div>
						  <div class="form-group">
						    <label>功能特色：</label>
						    <textarea rows="3" cols="50" name="function_profile"></textarea>
						  </div>
