...
<html lang="zh-CN">
  ...
  <script>
    ...
    function openaddEditModal(_addEditType) {
    	...
    	if('add' == _addEditType){
    		$('#formId fieldset legend').html('新增视频类型');
    	}else{
    		$('#formId fieldset legend').html('修改视频类型');
    	}
    ...
    function saveOrUpdate(){
	  ...
	  var _params = $('#formId').serialize();
   	  ...
   	  var _postUrl = '/videoType/addOrEdit.do';
	   ...
	   showConfirm('确认要删除'+rowDatas.length+'条数据吗？',
		function(){
		   ...
		   for(var i = 0;i<rowDatas.length ; i++){
			   _delIdArr.push(rowDatas[i].type_id);
	       }
		   $.post(path + '/videoType/delete.do',{ids:_delIdArr.toString()},function(_backData){
			   if("success" == _backData){
				   showOnlyMsg('删除成功！');
	   	  	   }else
			    ...
			    <div class="modal-content">
			      <div class="modal-body">
			        <form class="form-inline" id="formId">
			            ...
			            <input type="hidden" id="type_id" name="type_id" style="display: none;"/>
					    <fieldset>
					      <legend>新增视频类型</legend>
					      <h4 class="alert alert-danger showErrInfoP" style="display: none;"></h4>
					      
					      <div class="form-group">
						    <label for="nameInput">类型名称：</label>
						    <input type="text" class="form-control" name="name" id="nameInput" placeholder="视频类型名称" data-toggle="tooltip" data-placement="top" title="请输入视频类型名称！">
						  </div>
