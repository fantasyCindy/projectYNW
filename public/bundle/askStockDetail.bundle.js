...
/***/ (function(module, exports, __webpack_require__) {
	...
	var reduceTime = function reduceTime(time) {
	...
	$('.is_reply .is-reply-btn').click(function () {
	    //点击'去直播间看看'跳转值活跃榜第一名的直播间
	    queryFirstLiveRoom(function (data) {
	...
	window._bd_share_config = {
	    ...
	    share: {
	        ...
	        /* 增加浏览量 */
	    } };$.post("/consultation/addNoteReadCount.htm", { noteid: noteid }, function (back) {
	    ...
	    return {
	        init: function init() {
	            ...
	            container.on('click', '.column1', function () {
	                ...
	                if (type == "ask") {
	                    askWindow.render({
	                        filter: _.uniq(_.map(__answerInfo, function (item) {
	...
	var question = function () {
	    ...
	    if (ynIsTeacher) {
	        var questionuserid = container.find('.puber').data('id');
	...
	// 回答问题(只有老师端显示)
	var replyWindow = function replyWindow() {
	    ...
	    var submit = function submit() {
	        ...
	        $.post("/consultation/answerNote.htm", send, function (back) {
	            back = JSON.parse(back);
	            if (back.status == "1") {
	                layer.msg("发布成功");
	                setTimeout(function () {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = function () {
	    ...
	    var createItem = function createItem(item) {
	        return '<div class="title"><a href="' + item.href + '" target="_blank"><span class="name">' + item.name + '</span><span class="code">' + item.code + '</span></a><button class="addCustom"><span class="icon-plus">+</span>\u81EA\u9009</button></div><div class="content"><div class="imgw"><img src="' + item.src + '" /></div></div>';
	    };
	    ...
	    return {
	        ...
	        render: function render(ops) {
	            ...
	            //添加自选
	            container.on('click', '.addCustom', function () {
	                if (!ynIsLogin) return yn.login.render();
