	...
	var list = function () {
	    ...
	    var event = function event() {
	        ...
	        container.on('click', '.toTop', function () {
	            ...
	            if (typea >= 6 && typea <= 8) {
	                //头条类型 select的value为6,7,8时,传0(移动资讯);值为9,10,11时,传1(移动精选);值为1234时没有置顶功能
	                type = 0;
	        ...
	        container.on('click', '.post', function () {
	            ...
	            if (typea >= 6 && typea <= 8) {
	                //推送类型 select的value为6,7,8时,传1(移动资讯);值为9,10,11时,传2(移动精选);值为1234时没有推送功能
	                type = 1;
	            } else if (typea >= 9 && typea <= 11) {
	                type = 2;
	            }
	    ...
	    function compile(data) {
	        return data.map(function (item, i) {
	            return "<div class=\"item flex\"><input class=\"checkBox\" type=\"checkbox\" value=\"" + item.id + "\"><a href=\"" + item.link + "\" target=\"_blank\" class=\"title flex1\">" + item.title + "</a><span class=\"small category\">" + categorys[item.backnews_type] + "</span><span class=\"small time\">" + item.create_time + "</span><span class=\"small action\"><button class=\"edit fl\" data-id=\"" + item.id + "\" data-index=\"" + i + "\">\u7F16\u8F91</button><button class=\"delete fl\" data-id=\"" + item.id + "\" style=\"margin-left:8px;\">\u5220\u9664</button><button class=\"toTop style" + item.backnews_type + "\" data-id=\"" + item.backnews_id + "\">\u7F6E\u9876</button><button class=\"post style" + item.backnews_type + "\" data-type=\"" + item.backnews_type + "\" data-title='" + item.title + "' data-id=\"" + item.backnews_id + "\">\u63A8\u9001</button></div>";
	        }).join('');
	    }
	        ...
	        render: function render(ops) {
	            ...
	            if (data) {
	                ...
	                if (data.type >= 1 && data.type <= 4) {
	                    //分类 value1234为pc端资讯 678为移动端资讯 9 10 11为移动端精选
	                    $('#publish .first').click();
