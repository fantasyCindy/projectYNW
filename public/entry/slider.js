...
function slideLi() {
    ...
    //轮播到最前面时换成第二张图片 轮播到后面时换回第一张 
    $('.slide .img3 .three1').hide().siblings().show();
    $('.slide .img2 .three2').hide().siblings().show();
    $('.slide .img4 .three2').hide().siblings().show();
    //轮播图下面介绍的显示和隐藏
    var index = $('.slide .img3').index();
    console.log("index", index)
    var introduce = $('.lunbo-items li');
    for (var i = 0; i < introduce.length; i++) {
        $(introduce[i]).fadeOut(300);
    }
    $(introduce[index]).fadeIn(500);
...
function k_touch() {
    ...
    if (_content.attachEvent) {
        _content.attachEvent("touchstart", touchStart, false);
        _content.attachEvent("touchmove", touchMove, false);
        _content.attachEvent("touchend", touchEnd, false);
    }
