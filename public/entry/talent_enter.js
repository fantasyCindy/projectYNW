...
var addbox = (function() {
    var container;
    var creatDom = `<div class="add"></div>`

    return {
        init: function() {
            container = $('#Editor_editor1 .toolbar');
            container.append(creatDom);

            //点击时显示裁切框
            container.on('click', '.add', function() {
                var el = $(this);
                cropper.getInstance().render({ width: 148, height: 97, ratio: "free" });
                cropper.getInstance().onCrop = imageData => {
                    $.post("/auth/user/rzImgUpload.htm", {
                        ...
                        if (data.status == "success") {
                            ...
                            el.html(`<img src="${src}"/>`).addClass('yet');
                            container.append(creatDom);
                        }
                    }, 'json');
                }
...
var step1 = (function() {
    ...
    var event = function() {
        ...
        $('#applyType').on('click', 'li', function() {
            ...
            vocation = $(this).data('value')
            var isCompany = vocation == "1"
            isCompany ? $('#group-company').show() : $('#group-company').hide()
        })
            ...
            imgModel.get().render({
                callback: (val, info) => {
                    if (val == "yes") {
                        ...
                        // 发送短信
                        $.post("/sendPhoneCode.htm", { phone: moblie, phone_imgcode: info.value }, function(data) {
        ...
        next.on('click', function() {
            ...
            // 图片地址拼接
            var src = []
            $('#Editor_editor1 .add.yet img').each(function() {
                src.push($(this).attr('src'))
            })
            $('#zzzmimg').val(src.join(","))
